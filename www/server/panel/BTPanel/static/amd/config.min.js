							...
							return [4, this.$open({
								...
								success: function ($layer) {
									...
									$('#panel_menu_tab input').click(function () {
										...
										window.parent.postMessage('menu|' + id + '|' + Number($(this).prop('checked')), '*');
		...
		Config.prototype.switchPushType = function (row, isEdit) {
			...
			if (taskType.sorted) {
				taskType.sorted.forEach(function (findArray) {
					renderNum++;
					var firstStr = taskType.field.find(function (item) { return item.attr == findArray[0]; });
					lineData = {
						label: firstStr.name,
						group: []
					};
					findArray.forEach(function (findItem, index) {
						...
						var templateConfig = taskType.field.find(function (item) { return item.attr == findItem; }), fieldItem = $.extend(true, {}, templateConfig);
						if (isEdit)
									...
									if (childGroup['type'] == 'select' || childGroup['type'] == 'radio') {
										...
										if (childGroup['type'] == 'radio' && formData.tid == 'system_push@0') {
											childGroup['event'] = function (formD, element, that) {
												var disk = that.config.form[1].group[0].list.find(function (item) { return item.value == formD.project; });
