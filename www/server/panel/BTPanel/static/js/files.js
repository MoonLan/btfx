...
var bt_file = {
						...
						$.post('/tamper_core/get_effective_path.json', {path: path}, function (res) {
							if(!res.status) {
								...
								if(data.type == 'dir') {
									...
									var arr_suffix = String(data.filename).indexOf('.') !== -1 ?data.filename.split('.'):''
									suffix =  String(data.filename).indexOf('.') !== -1 ? '.' + arr_suffix[arr_suffix.length - 1] : ''
  ...
  set_menu_line_view_resize: function () {
    ...
    //排除磁盘、批量操作的宽度
    $('.file_nav_view .nav_group').not('.mount_disk_list,.nav_group.multi').each(function () {
    ...
    if ($('.file_list_content .file_tr.active').length > 1) {
      ...
      if (surplusWdith  < batch_list_width) {
        ...
        $('.batch_group_list>.nav_btn_group').slice(0, batchNumber-1).show();
  ...
  set_rsync_view: function (path, editConfig) {
    ...
    layer.open({
      ...
      success: function (layers) {
				$(layers).find('.layui-layer-content').css('height','230px')
              ...
              group: [
                {
                  ...
                  event: function (formData) {
                    ...
                    var rdata = {
                    	...
                    	ext: formData.ext,
                    	path: formData.path,     //路径
                    	is_sub: formData.is_sub,    //0不包含子目录 1 包含子目录
                    	s_time: formData.s_time, // 开始时间
                    	e_time: formData.e_time, // 结束时间
                    	min_size: formData.min_size, // 最小大小
                    	max_size: formData.max_size, // 最大大小
											model_index: "files",
                    	mod_name:"search",
                    	def_name:"get_search_result",
                    	ws_callback:"111"
                    }
                    loadT = bt.load("<div class='my_file_speed'>正在文件搜索，请稍候...</div>");
                    getFlieSpeed('.my_file_speed','/ws_model','search',rdata,function(resData){
                    	loadT.close()
                    	fileThat.renderSearchResult(resData.result,JSON.stringify(formData),_type)
                    });  
                  }
                }
              ]
  ...
  searchReplaceContent: function () {
    var that = this,
      ...
      data = {
        ...
        mod_name:"files",
        def_name:"files_search",
        ws_callback:"1"
        }
  	loadT = bt.load("<div class='my_file_speed'>正在文件搜索，请稍候...</div>");
  	getFlieSpeed('.my_file_speed','/ws_panel','files',data,function(info){
    	loadT.close()
      var res = info.result
      var reg = new RegExp("(" + that.escodeChange(data.text) + ")");
      if (res && res['error']) return layer.msg(res.error, { icon: 2 });
 ...
 renderSearchResult: function (list, sCon,type) {
	...
	_box.html('')
	if (type != 'dir') {
		itemList +='<div id="file-search-result-body" class="file-search-result-body">'
	...
	if(JSON.stringify(list) == "{}"){
					_box.html('<div style=" color: #bcbcbc; font-size: 16px; text-align: center; margin-top: 50px">沒有搜索到任何数据</div>')
				}else{
		...
		_box.find('.editFile').click(function () {
			openEditorView(0, $(this).data('filename'), function (val, aceEitor) {
				aceEditor.ace.find(JSON.parse(sCon).search[0])
				aceEditor.ace.execCommand('find')
			})
		})
