...
var site = {
            ...
            label: '项目端口',
              ...
              change: function (form, value, val, field) {
								setTimeout(function () {//延迟校验端口,解决点击放行端口无效问题
									if(form.port !== '') {
										bt_tools.send({url: 'project/nodejs/advance_check_port', data: {port: form.port}}, function (res) {
											if (!res.status){
												$('.port_input').val('')
      ...
      site_table = bt_tools.table({
        ...
        success: function (that) {
          ...
          if(!$('.pull-left button:eq(5) .icon_title').length) {
					...
					$('#bt_site_table .pull-left button').eq(5).before('<div class="inlineBlock dividing-line" style="margin: 0 10px 0 5px;"></div>')
                ...
                bt.site.set_endtime(item.id, date, function (res) {
                  ...
                  bt.msg(res);
					...
					window.onresize = function () {
						$('#bt_site_table .pull-right .bt_search input').css('width', '230px');
  ...
  edit: {
    ...
    set_composer: function (web) {
      ...
      $.post('/files?action=get_composer_version', { path: web.path }, function (v_data) {
        ...
        var com_body = '<from class="bt-form" style="padding:30px 0;display:inline-block;width: 630px;">' +
            ...
            '<div class="line" style="clear:both"><span style="width: 105px;" class="tname"> </span><div class="info-r"><button '+(msg.includes('没有找到')?'disabled':'')+' class="btn '+(msg.includes('没有找到')?'btn-default':'btn-success')+' btn-sm" onclick="site.edit.exec_composer()">执行</button></div></div>' +
            '</from>' +
          ...
          success: function (that) {
						if(authentication_table){
							...
							if($('.mutual_ssl').length == 0){
								...
								$('.config_ssl_info').unbind('click').click(function(){
    ...
    set_config: function (web) {
      var con = '<p style="color: #666; margin-bottom: 7px">提示：Ctrl+F 搜索关键字，Ctrl+S 保存，Ctrl+H 查找替换</p><div class="bt-input-text ace_config_editor_scroll" style="height:580px; line-height:18px;" id="siteConfigBody"></div>\
				...
				<div id="form_redirect" class="bt-form-new pd20">\
					...
					<div class="form-inline redirectpath" style="display: none; flex-wrap: nowrap;">\
						<div class="form-item">\
							...
							<div class="form-value">\
							<input class="bt-input-text" name="redirectpath" placeholder="如: http://'+sitename+' " type="text" style="width: 200px;" />\
							</div>\
						</div>\
						<div class="form-item">\
							<div class="form-label" style="width: 80px;">目标URL</div>\
          ...
          callback: function(content) {
            ...
            var state = $('<div class="ssl_state_info ' + (!rdata.csr ? 'hide' : '') + '">' +
                ...
                '<div class="state_item"><span>到期时间：</span><span style="'+
								...
								'"  class="' + (expirationTime >= 30 ? 'btlink' : '') + '">' + 
			...
			$.post("plugin?action=get_soft_find", {
				...
				if (rdata.task == "-1") {
					...
					var pan = '<span id="runPanel" style="cursor:pointer">' +
						'<img src="/static/img/soft_ico/ico-' + rdata.name + '.png">' +
						...
						'</span>';
					$('.pull-left button').eq(6).css({'padding':'3px 10px','height':'30px','box-sizing':'border-box'})
					if(!rdata.status){
						$('.pull-left button').eq(6).addClass('stopStatus')
					}
					$('.pull-left button').eq(6).html(pan);
          ...
          manu.append('<div class="ac" onclick="bt.pub.set_server_status(\'' + rdata.name + '\',\'' + 'reload' + '\')">' + '重载' + '</div>');
					...
					var div=$('.Resize')
						if(window.innerWidth<1600 &&window.innerWidth>1400){
							$('.pull-left button').eq(6).show()
							div.html(rdata.title)
						}else if(window.innerWidth<=1400){
							$('.pull-left button').eq(6).hide()
						}else{
							$('.pull-left button').eq(6).show()
