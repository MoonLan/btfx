...
var database = {
  ...
  database_detail: function (id, dataname,db_type) {
    if(bt.data.db_tab_name == 'mysql'){
      ...
      var loadT = bt.load(lan.public.the_get),p = 1
      if(typeof db_type === 'number') p = db_type 
      bt.pub.get_data('table=backup&search=' + id + '&limit=5&type=1&tojs=database.database_detail&p='+p, function (frdata) {
        loadT.close();
