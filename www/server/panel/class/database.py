...
class database(datatool.datatools):
    ...
    def CheckDatabaseStatus(self, get):
        ...
        if sid != 0:
            ...
            db_host = "localhost"
            try:
                myconf = public.readFile(self._MYSQL_CNF)
                rep = r"port\s*=\s*([0-9]+)"
                db_port = int(re.search(rep, myconf).groups()[0])
            except:
                db_port = 3306
        mysql_obj = db_mysql.panelMysql().set_host(db_host, db_port, db_name, db_user, root_password)
        db_status = not isinstance(mysql_obj, bool)
    ...
    def AddDatabase(self, get):
        ...
        if len(username) > 64: return public.returnMsg(False, '数据库用户名不能超过32位')
        ...
        # 从MySQL验证是否存在
        if self.database_exists_for_mysql(mysql_obj, data_name):  return public.returnMsg(False,
                                                                                          '指定数据库已在MySQL中存在，请换个名称!')
    ...
    # 创建用户
    def __CreateUsers(self, sid, dbname, username, password, address):
        mysql_obj = public.get_mysql_obj_by_sid(sid)
        if not mysql_obj: return public.returnMsg(False, '连接指定数据库失败')
...
'''
    ...
    def DeleteDatabase(self, get):
        ...
        id = get['id']
        name = get['name']
        find = public.M('databases').where("id=? AND LOWER(type)=LOWER('mysql')", (id,)).field(
        ...
        sid = find['sid']
        if find['db_type'] in ['0', 0] or sid:  # 删除本地数据库
            if os.path.exists('data/recycle_bin_db.pl') and not sid: return self.DeleteToRecycleBin(name)
            accept = find['accept']
            ...
            mysql_obj.execute("flush privileges")
        # 删除SQLITE
        public.M('databases').where("id=? AND LOWER(type)=LOWER('mysql')", (id,)).delete()
    ...
    def DeleteToRecycleBin(self, name):
        import json
        data = public.M('databases').where("name=? AND LOWER(type)=LOWER('mysql')", (name,)).field(
        ...
        public.M('databases').where("name=? AND LOWER(type)=LOWER('mysql')", (name,)).delete()
        public.WriteLog("TYPE_DATABASE", 'DATABASE_DEL_SUCCESS', (name,))
    ...
    def RecycleDB(self, filename):
        ...
        self.__CreateUsers(0, data['name'], data['username'], data['password'], data['accept'])
    ...
    def ToBackup(self, get):
        ...
        db_find = public.M('databases').where("id=? AND LOWER(type)=LOWER('mysql')", (id,)).find()
        name = db_find['name']
        fileName = f"{name}_mysql_data_{time.strftime('%Y%m%d_%H%M%S', time.localtime())}.sql.gz"
    ...
    def InputSql(self, get):
        ...
        db_find = public.M('databases').where("name=? AND LOWER(type)=LOWER('mysql')", name).find()
    ...
    def SyncGetDatabases(self, get):
        ...
        for value in data:
            ...
            if sql.table('databases').add('name,sid,db_type,username,password,accept,ps,addtime', (value[0], sid, db_type, value[0], '', host, ps, addTime)): n += 1
    ...
    def SetDatabaseAccess(self, get):
        ...
        password = public.M('databases').where("username=? AND LOWER(type)=LOWER('mysql')", (name,)).getField('password')
        result = mysql_obj.query("show databases")
        isError = self.IsSqlError(result)
        if isError != None: return isError
        users = mysql_obj.query("select Host from mysql.user where User='" + name + "' AND Host!='localhost'")
