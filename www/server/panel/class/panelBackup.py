...
class backup:
    ...
    def backup_path(self, spath, dfile=None, exclude=[], save=3, echo_id=None):
        try:
            ...
            if self._cloud is None:
                ...
                csu = CloudStoraUpload()
                self._cloud = csu.run(cloud_name)
    ...
    def backup_database(self, sName, save, echo_id: str):
        ...
        if self._cloud is not None:
            ...
            if self._cloud_new.cloud_upload_file(backup_path, upload_path):
                self.echo_info("已成功上传到{}".format(self._cloud._title))
            else:
                error_msg = "备份任务执行失败。"
                if hasattr(self._cloud, "error_msg"):
        ...
        # 清理多余备份
        if self._cloud is not None:
            backups = public.M("backup").where("cron_id=? and type=? and pid=? and filename LIKE ?", (self.cron_info["id"], "1", database["id"], "%{}%".format(self._cloud._name))).field("id,name,filename").select()
        else:
    ...
    def mysql_backup_database(self, db_find: dict, args: dict) -> Tuple[bool, str]:
        ...
        mysql_obj = db_mysql.panelMysql().set_host(db_host, db_port, db_name, db_user, db_password)
        ...
        backup_shell = "'{mysqldump_bin}' --routines --events --skip-triggers --default-character-set='{db_charset}' --force " \
                       "--hex-blob --opt --user={db_user} --host='{db_host}' --port={db_port} '{db_name}'".format(
    ...
    def mongodb_backup_database(self, db_find: dict, args: dict) -> Tuple[bool, str]:
        ...
        if db_find["db_type"] == 0:
            if panelMongoDB.get_config_options("authorization", str, "disabled") == "enabled":
                ...
                db_password = None
            db_port = int(panelMongoDB.get_config_options("port", int, 27017))
        ...
        db_storage_size = 0
        try:
            sql_obj = mongodb_obj.get_db_obj(db_name)
            data = sql_obj.command("dbStats")
    ...
    def redis_backup_database(self, db_find: dict, args: dict) -> Tuple[bool, str]:
        ...
        backup_path = os.path.join(self._REDIS_BACKUP_DIR, file_name)
