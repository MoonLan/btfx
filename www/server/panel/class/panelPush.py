...
class panelPush:
    ...
    def set_push_config(self, get):
        ...
        if 'title' in json.loads(get['data']).keys():
            ...
            title = 'type'
        res = public.WriteLog('告警设置', '添加告警任务【{}】'.format(json.loads(get['data'])[title]))
        if not module in p_list:
            return public.returnMsg(False, '指定模块{}未安装.'.format(module))
        ...
        pdata = self.__get_args(pdata, 'push_count', 0, type_list=(int, float))
    ...
    def del_push_config(self, get):
        ...
        module = get.name
        data = self._get_conf()
        res = public.WriteLog('告警设置', "删除告警任务【{}】".format(data[module][id]['title']))
